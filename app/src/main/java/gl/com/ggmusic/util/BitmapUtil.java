...
public final class BitmapUtil {
	 ...
	 *            目标高度
	 */
	public static Bitmap getBitmapFromByteArray(byte[] data, int offset, int length, int reqWidth,
			int reqHeight) {
		BitmapFactory.Options options = new BitmapFactory.Options();
		options.inJustDecodeBounds = true;
	...
	public static Bitmap createThumbnailBitmap(Bitmap bitmap, Context context) {
		...
		if (width > 0 && height > 0) {
			if (width < bitmapWidth || height < bitmapHeight) {
				...
				final Config c = (width == sIconWidth && height == sIconHeight)
						? bitmap.getConfig()
						: Config.ARGB_8888;
				...
				paint.setDither(false);
				paint.setFilterBitmap(true);
				canvas.drawBitmap(bitmap, (sIconWidth - bitmapWidth) / 2, (sIconHeight - bitmapHeight) / 2,
						paint);
				return thumb;
			}
		}
	...
	public static Bitmap blur(Context context, Bitmap sentBitmap, int radius) {
		...
		if (radius < 0) {
			radius = 0;
			if (DEBUG) {
				// LogUtils.w(TAG, "radius must be 0 < r < 25 , forcing
				// radius=0");
			}
		} else if (radius > 25) {
			radius = 25;
